...
/**
 * {@link RSocketStrategiesCustomizer} that adds a custom extractor to the {@link
 * org.springframework.messaging.rsocket.MetadataExtractorRegistry}. We're using here an alternate
 * metadata format that uses a JSON object to hold multiple metadata entries with a custom JSON
 * media type.
...
public class JsonMetadataStrategiesCustomizer implements RSocketStrategiesCustomizer {
    ...
    public static final MimeType METADATA_MIME_TYPE =
        MimeType.valueOf("application/vnd.spring.rsocket.metadata+json");
